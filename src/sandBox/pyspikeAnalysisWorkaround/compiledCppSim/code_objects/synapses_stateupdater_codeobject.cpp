...
const int _numw = _dynamic_array_synapses_w.size();
 ...
 const double _lio_1 = (0.4 * dt) / ((5 * 50) * 1.0);
 ...
 const double _lio_3 = dt / 1.0;
	...
	for(int _idx=0; _idx<_N; _idx++)
	{
        ...
        const double _w = ((_lio_1 * ((R_hat * w) * (1.0 + (_lio_2 * (- R_hat))))) / _brian_abs((- 1.0) + (_lio_2 * R_hat))) + w;
        const double _R_hat = (_lio_3 * (- R_hat)) + R_hat;
        w = _w;
        R_hat = _R_hat;
