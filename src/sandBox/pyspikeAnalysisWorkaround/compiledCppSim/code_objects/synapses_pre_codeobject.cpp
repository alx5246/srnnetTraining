...
const int _numapre = _dynamic_array_synapses_apre.size();
 ...
 const double _lio_1 = 1.0 / 0.06;
 const double _lio_2 = 0.0 - t;
 const double _lio_3 = 1.0 / 0.02;
 const double _lio_4 = 1.0 / (5 * 50);
 const double _lio_5 = 1.0 / 6.6425;
   ...
   const double R_hat = _ptr_array_synapses_R_hat[_idx];
   ...
   apost *= exp(_lio_1 * (_lio_2 + lastupdate));
   apre *= exp(_lio_3 * (_lio_2 + lastupdate));
   L += w;
   apre += 0.0002;
   const double K = (_lio_4 * R_hat) / _brian_abs(1.0 - (_lio_5 * R_hat));
   w += 1 * (K * apost);
   lastupdate = t;
