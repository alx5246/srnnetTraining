...
const int _num_clock_t = 1;
 ...
 double*   _ptr_array_statemonitor_clock_t = _array_statemonitor_clock_t;
    ...
    _dynamic_array_statemonitor_K.resize(_new_size, _num_indices);
    _dynamic_array_statemonitor_R_hat.resize(_new_size, _num_indices);
    _dynamic_array_statemonitor_w.resize(_new_size, _num_indices);

    // scalar code
    const int _vectorisation_idx = -1;
        
    const double _lio_1 = 1.0 / (5 * 50);
    const double _lio_2 = 1.0 / 6.6425;
    ...
    for (int _i = 0; _i < _num_indices; _i++)
    {
        ...
        const int _vectorisation_idx = _idx;
                    ...
                    const double _source_K = (_lio_1 * __source_K_synapses_R_hat) / _brian_abs(1.0 - (_lio_2 * __source_K_synapses_R_hat));
                    const double _to_record_K = _source_K;



            _dynamic_array_statemonitor_K(_new_size-1, _i) = _to_record_K;
            _dynamic_array_statemonitor_R_hat(_new_size-1, _i) = _to_record_R_hat;
